...
def load_data():
    ...
    return df
data = load_data()
# --- Unique choices from the dataset
...
model = load_pickle("salary_model.pkl")
...
with st.form("prediction_form"):
    ...
    with col1:
        experience = st.number_input(
            "Years of Experience:",
            int(data["Experience"].min()),
            int(data["Experience"].max()),
            int(data["Experience"].mean())
        )
        age = st.number_input(
            "Age:",
            int(data["Age"].min()),
...
st.markdown("---")
st.markdown(
    "<p style='font-size:16px;text-align:center;'>Made with ❤ using Streamlit</p>",
